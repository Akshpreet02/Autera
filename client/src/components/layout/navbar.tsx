...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm shadow-md transition-all duration-300">
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link href="/" className="flex-shrink-0 flex items-center">
              <span className="text-gray-800 font-bold text-xl tracking-tight">Autera</span>
            </Link>
          </div>
          ...
          <div className="hidden md:flex md:items-center md:space-x-1">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="text-gray-600 hover:text-primary hover:bg-gray-100 px-4 py-2 rounded-md text-sm font-normal transition-colors duration-200"
              >
                {item.label}
              </a>
            ))}
      ...
      <div
        className={cn(
          "md:hidden absolute w-full bg-white/95 backdrop-blur-sm transform transition-all duration-300 ease-in-out",
        ...
        <div className="px-4 pt-3 pb-5 space-y-1">
          {navItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="block px-3 py-2 rounded-md text-base font-normal text-gray-600 hover:text-primary hover:bg-gray-100"
